      ...
      <div class="dialog-content">
        <div class="dialog-header">
          ...
          <div v-if="store.isLogin" class="dialog-buttons">
            <button v-if="isContractDeposit" @click.prevent="deleteDepositUser" class="custom-button">가입 취소하기</button>
            <button v-else @click.prevent="addDepositUser" class="custom-button">가입하기</button>
          </div>
        </div>

        <table class="dialog-table">
          <tbody>
            <tr v-for="(value, key) in selectedDeposit" :key="key" class="dialog-table-row">
...
const isContractDeposit = computed(() => {
  ...
  const contractDepositArray = Array.isArray(userInfo.deposits) ? userInfo.deposits : [];
  return contractDepositArray.some(e => e['fin_prdt_cd'] === selectedDepositCode.value);
});
...
const addDepositUser = function () {
  ...
  .then((res) => {
    ...
    .catch((err) => {
      console.log('뽑힌거:',selectedDepositCode.value)
